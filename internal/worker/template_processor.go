...
func (p *TemplateProcessor) processItem(ctx context.Context, item *queue.WorkItem) error {
	...
	kubeTemplate.Status.ProcessingPhase = "Processing"
	...
	if err != nil {
		kubeTemplate.Status.ProcessingPhase = "Failed"
		kubeTemplate.Status.Status = fmt.Sprintf("Error: %v", err)
		now := time.Now()
		kubeTemplate.Status.ProcessedAt = &now
		if statusErr := p.Client.Status().Update(ctx, &kubeTemplate); statusErr != nil {
	...
	for _, template := range kubeTemplate.Spec.Templates {
		...
		if !allowed {
			log.Info("Resource not allowed by policy", "gvk", gvk)
			kubeTemplate.Status.ProcessingPhase = "Failed"
		...
		if len(matchedRule.TargetNamespaces) == 0 {
			...
			kubeTemplate.Status.ProcessingPhase = "Failed"
			kubeTemplate.Status.Status = fmt.Sprintf("Error: Resource %s has no target namespaces", gvk.String())
			now := time.Now()
			kubeTemplate.Status.ProcessedAt = &now
			if err := p.Client.Status().Update(ctx, &kubeTemplate); err != nil {
				log.Error(err, "Failed to update status")
		...
		if !contains(matchedRule.TargetNamespaces, obj.GetNamespace()) {
			...
			kubeTemplate.Status.ProcessingPhase = "Failed"
			kubeTemplate.Status.Status = fmt.Sprintf("Error: namespace %s not allowed for %s", obj.GetNamespace(), gvk.String())
			now := time.Now()
			kubeTemplate.Status.ProcessedAt = &now
			if err := p.Client.Status().Update(ctx, &kubeTemplate); err != nil {
				log.Error(err, "Failed to update status")
		...
		if matchedRule != nil && matchedRule.Rule != "" {
			if valid, err := p.validateWithCEL(matchedRule.Rule, obj.Object); err != nil {
				...
				kubeTemplate.Status.ProcessingPhase = "Failed"
				kubeTemplate.Status.Status = fmt.Sprintf("Error: CEL validation failed for %s: %v", gvk.String(), err)
				now := time.Now()
				kubeTemplate.Status.ProcessedAt = &now
				if statusErr := p.Client.Status().Update(ctx, &kubeTemplate); statusErr != nil {
					log.Error(statusErr, "Failed to update status")
				...
				kubeTemplate.Status.ProcessingPhase = "Failed"
				kubeTemplate.Status.Status = fmt.Sprintf("Error: Resource %s failed CEL validation", gvk.String())
				now := time.Now()
				kubeTemplate.Status.ProcessedAt = &now
				if statusErr := p.Client.Status().Update(ctx, &kubeTemplate); statusErr != nil {
					log.Error(statusErr, "Failed to update status")
		...
		if err := p.Client.Patch(ctx, &obj, client.Apply, client.FieldOwner(fieldManager)); err != nil {
			if errors.IsInvalid(err) && template.Replace {
				...
				kubeTemplate.Status.ProcessingPhase = "Failed"
				kubeTemplate.Status.Status = fmt.Sprintf("Error: Failed to apply %s/%s: %v", gvk.String(), obj.GetName(), err)
				now := time.Now()
				kubeTemplate.Status.ProcessedAt = &now
				if statusErr := p.Client.Status().Update(ctx, &kubeTemplate); statusErr != nil {
					log.Error(statusErr, "Failed to update status")
	...
	kubeTemplate.Status.ProcessingPhase = "Completed"
