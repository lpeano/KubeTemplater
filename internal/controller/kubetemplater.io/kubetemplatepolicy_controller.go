...
// the KubeTemplatePolicy object against the actual cluster state, and then
...
func (r *KubeTemplatePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// The KubeTemplatePolicy controller does not reconcile any resources on its own.
	// It is the KubeTemplate controller that reads the policies and enforces them.
	// This controller is here to ensure that the CRD is installed and that the
	// necessary RBAC permissions are in place.
