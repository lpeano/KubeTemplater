...
func main() {
	...
	// CACHE_TTL: Policy cache time-to-live in seconds (default: 300 = 5 minutes)
	...
	// Initialize policy cache with configurable TTL
	policyCache := cache.NewPolicyCache(mgr.GetClient(), cacheTTL)
	setupLog.Info("Policy cache initialized", "ttl", cacheTTL)

	// Initialize work queue for async processing with configurable retry parameters
	workQueue := queue.NewWorkQueueWithConfig(queueMaxRetries, queueInitialRetryDelay, queueMaxRetryDelay)
