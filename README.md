...
**RBAC Security & Event Notifications** - Granular security controls and better observability:

### 🔒 Key Features
- **Kubernetes Events**: Warning events on template auto-pause for better observability
- **RBAC Security**: `allowClusterResources` flag (default: `false`) for multi-tenant safety
- **Webhook Protection**: ValidatingWebhookConfiguration restricted by resourceNames
- **Fixed Reconciliation**: Failed templates properly respect cooldown and spec-change detection

### 🎯 RBAC Configuration
```yaml
rbac:
  allowClusterResources: false  # Default: secure (only namespaced resources)
  # Set to true for ClusterRoles, PV, Namespaces, CRDs
```

### 📊 View Template Events
```bash
kubectl describe kubetemplate <name>
...
**Anti-Loop Protection & Spec Versioning** - Prevents infinite retry loops and eliminates false drift positives:

### 🛡️ Key Features
- **MaxRetryCycles**: Templates pause after persistent failures (~15 min), resume via annotation
- **AppliedSpecHash**: SHA256 tracking detects real spec changes vs transient errors
- **SSA Dry-Run**: Eliminates false positives from Kubernetes field normalizations
- **Paused Status**: Clear visibility when templates stop processing with reason
- **Race Condition Fix**: Robust status updates prevent templates from being stuck after validation

### 🎯 Resume Paused Templates
```bash
kubectl annotate kubetemplate <name> kubetemplater.io/resume=true
```
...
tuning:
  queue:
    maxRetryCycles: 3  # Default: 3 (~15min), 0 = unlimited
```
...
- **CACHE_TTL**: General cache lifetime (60-600s, default: 300s)
- **POLICY_CACHE_TTL**: Policy cache for security-critical operations (30-600s, default: 60s)
...
- **QUEUE_MAX_RETRY_CYCLES**: Max retry cycles before pause (0-10, default: 3, 0=unlimited)
