...
### 🔒 RBAC Security Improvements & Event Notifications

**Security enhancements and better observability** - Granular RBAC controls and Kubernetes event notifications.

#### Added

- **Kubernetes Event Notifications**
  - EventRecorder integration for worker events
  - Warning events emitted when templates auto-pause (max retry cycles exceeded)
  - Events visible via `kubectl describe kubetemplate <name>` and `kubectl get events`
  - Better observability for operations and troubleshooting
  - RBAC permissions added: `events` (create, patch)

- **RBAC Security Enhancements**
  - New `rbac.allowClusterResources` configuration flag (default: `false`)
  - When `false`: Restricts to namespaced resources only (Deployments, Services, ConfigMaps, Secrets, etc.)
  - When `true`: Allows cluster-scoped resources (ClusterRoles, PersistentVolumes, Namespaces, CRDs)
  - Recommended: `false` for multi-tenant environments, `true` for platform/infrastructure management
  - ValidatingWebhookConfiguration access restricted with `resourceNames`
  - Prevents unauthorized modification of other webhooks in the cluster
  - **Breaking**: Default changed to `false` for secure-by-default posture

#### Fixed

- **Failed Template Reconciliation**
  - Fixed Failed templates not being re-queued properly
  - Proper cooldown period respect for Failed templates without spec changes
  - Spec-change detection triggers immediate retry with counter reset
  - Templates no longer stuck indefinitely in "Queued" state after failures
  - WorkQueue retry mechanism properly enforced for Failed templates

#### Changed

- **RBAC Default Behavior**
  - `allowClusterResources` now defaults to `false` (was: implicit `true`)
  - Existing deployments: Set `rbac.allowClusterResources: true` in values.yaml to maintain current behavior
  - New deployments: More secure by default, prevents accidental cluster-wide privilege escalation

#### Security

- Restricted ValidatingWebhookConfiguration to specific resource name
- Configurable cluster-scoped resource permissions
- Secure by default for multi-tenant environments
- Principle of least privilege enforced

### Migration Guide

If you need cluster-scoped resource creation (ClusterRoles, PV, Namespaces, CRDs), add to your `values.yaml`:

```yaml
rbac:
  allowClusterResources: true  # Enable cluster-scoped resources
```

Otherwise, no action needed - secure defaults apply.

## [0.6.1] - 2025-12-17

### 🛡️ Anti-Loop Protection & Spec Versioning

**Reliability improvements** - Prevents infinite retry loops and eliminates false drift positives.

#### Fixed

- **Race Condition in Status Updates**
  - Fixed critical race condition during KubeTemplate creation
  - Controller now uses `retry.RetryOnConflict` with exponential backoff for robust status updates
  - Worker defensively handles templates with empty or Queued phase
  - Templates are never lost even on API conflicts or transient errors
  - Prevents templates from being stuck after webhook validation
  - Retry logic: 10ms → 20ms → 40ms with automatic fallback

#### Added

- **Anti-Loop Protection with MaxRetryCycles**
  - Configurable `QUEUE_MAX_RETRY_CYCLES` (default: 3, 0 = unlimited)
  - Templates automatically pause after max retry cycles (~15 minutes with defaults)
  - Prevents infinite retry loops on persistent errors
  - Resume via annotation: `kubectl annotate kubetemplate <name> kubetemplater.io/resume=true`

- **Spec Versioning with AppliedSpecHash**
  - SHA256 hash tracking of applied template specs
  - Detects actual template spec changes vs transient errors
  - Enables smarter reconciliation decisions
  - Auto-populated on first reconcile (backward compatible)

- **SSA Dry-Run Drift Detection**
  - Server-Side Apply dry-run for drift detection
  - Eliminates false positives from Kubernetes normalizations
  - More accurate drift detection without spurious updates
  - `dryRunChecks` counter tracks periodic reconciliation operations

- **Paused Status for Templates**
  - New `Paused` processing phase
  - `pausedAt` timestamp field
  - `pausedReason` explanation field
  - Clear visibility into why templates stopped processing

#### Changed

- **CRD Schema Updates**
  - Added `appliedSpecHash` to status
  - Added `pausedAt` timestamp to status
  - Added `pausedReason` string to status  
  - Added `dryRunChecks` counter to status

- **Helm Chart Configuration**
  - Added `tuning.queue.maxRetryCycles` parameter (default: 3)
  - Environment variable: `QUEUE_MAX_RETRY_CYCLES`

#### Fixed

- Infinite retry loops on persistent template errors
- False drift detection from Kubernetes field normalizations
- Unnecessary reconciliation cycles when specs haven't changed
