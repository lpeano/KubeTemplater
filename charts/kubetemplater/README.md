...
All performance parameters are configurable in `values.yaml` under the `tuning` section:

```yaml
tuning:
  # Worker pool size (1-20, recommended: 3-10)
  numWorkers: 3
  
  # Cache TTL in seconds (60-600, recommended: 180-300)
  cacheTTL: 300
  
  # Drift detection interval in seconds (30-300)
  periodicReconcileInterval: 60
  
  # Queue retry configuration
  queue:
    maxRetries: 5              # Max attempts before cooldown (1-10)
    initialRetryDelay: 1       # Initial delay in seconds (1-10)
    maxRetryDelay: 300         # Max delay cap in seconds (60-600)
```

### Tuning by Scale

**Small Deployment (< 5,000 templates)** - Default values are optimal
...
### From v0.5.x or earlier to v0.6.0

**New in v0.6.0:** Configurable performance tuning parameters

The upgrade is backward compatible. Default values match previous hardcoded behavior.
...
**Optional:** Tune performance for your deployment scale:
