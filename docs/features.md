...
kind: KubeTemplatePolicy
metadata:
  name: myapp-keycloak-policy
  namespace: kubetemplater-system
spec:
  sourceNamespace: myapp-dev
  validationRules:
    - group: "v1.edp.epam.com"
      kind: KeycloakRealm
      version: v1
      targetNamespaces: ["keycloak-prod"]
      fieldValidations:
        - name: realm-name-validation
          type: cel
          fieldPath: "metadata.name"
          cel: "value == 'myapp-realm'"
          message: "KeycloakRealm name must be 'myapp-realm'"
        ...
        - name: keycloak-ref-name-validation
          ...
          cel: "value == 'cluster-keycloak'"
          message: "keycloakRef.name must be 'cluster-keycloak'"
```
...
- **Cross-namespace** template creation (myapp-dev → keycloak-prod)
