    ...
    - group: "v1.edp.epam.com"
      ...
      fieldValidations:
        - name: realm-name-validation
          type: cel
          fieldPath: "metadata.name"
          cel: "value == 'acme-realm'"
          message: "KeycloakRealm name must be 'acme-realm'"
        ...
        - name: keycloak-ref-name-validation
          ...
          cel: "value == 'cluster-main'"
          message: "keycloakRef.name must be 'cluster-main'"
```
...
- **Cross-namespace** template creation (app-dev → keycloak-uat)
