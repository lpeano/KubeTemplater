...
Once a `KubeTemplate` is accepted, the controller retrieves the matching `KubeTemplatePolicy` from the operator namespace. The policy is matched based on the `sourceNamespace` field matching the `KubeTemplate`'s namespace.
